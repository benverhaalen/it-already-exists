...
When mocks pass but native execution fails, inspect the difference between the mock and the real API contract before changing the target. Callback objects can have read-only properties, and returned handle objects can be recycled after the callback. A plain mutable object hides both constraints. Reproduce the observed failure with a faithful contract fixture, verify the correction there, and then run the complete native control. Treat mock coverage, native lifecycle coverage and success in the target as separate evidence.
