...
Choose inputs for which the competing explanations predict different outputs. Zero-valued data, neutral colors or a symmetric example can make different formulas produce the same result, leaving a passing test unable to distinguish them. Write down those predictions before execution and include a non-neutral case. If every tested case matches, retire the failure mechanism within that tested scope; do not infer the target's unseen inputs or apply a speculative repair. Keep the negative result so the next experiment tests a different unresolved assumption.

Preserve representation when transferring an observed input into a control. Record raw scalar words alongside their declared encoding, units and decoded values. A coordinate can be fixed-point, normalized or transformed; visible pixel dimensions do not establish its internal units. Observe the setup sequence from a known boundary, validate the decoder with independently authored values, and derive only what that history supports. A returned setter call alone does not prove the backend accepted it.

If a simple control passes while the target still fails, narrow the test to the operation sequence actually observed at the permitted boundary. The same data type can take different implementation paths: creating an image with its contents and allocating it before updating its contents should produce equivalent pixels, but a backend may handle them differently. Reproduce both paths with owned inputs and the same expected output. Then vary one relevant factor at a time, such as format or update extent, while retaining a known-good case. Compare actual output with the prediction, not merely an error code or successful call. Keep changes to the observer separate from changes to the operation under test.

A selective failure with passing controls is useful evidence even while the target remains unusable. It narrows the suspect path; it does not establish which component caused the defect. Choose the next experiment to separate the remaining explanations, rather than repeat the whole launch or discard the result as inconclusive. If the positive control also fails, repair the test setup before interpreting the target-specific case.

Freeze the exact probe or adapter revision used for each run. Record its content hash, effective inputs and backend with the output; an edited local file may differ from the code already loaded into a process. Compare results against the executed revision, and treat a changed probe as a new experiment.

Include the observer's complete lifecycle in the native control. Installing an adapter after a resource already exists does not test the wrapper that must recognize its creation, ownership and later use. Arm that wrapper before creating an independently owned resource, require it to select that exact resource, then exercise the intended operation and cleanup. Restrict selection so unrelated target activity cannot become the test subject.

When mocks pass but native execution fails, inspect the difference between the mock and the real API contract before changing the target. Before an expensive reset, a small read-only capability check can catch missing exports, unsupported options or a mismatched runtime. Check exact names and signatures against the public contract; a mock generated from the same mistaken list can agree with the probe. Callback objects can also have read-only properties, and returned handle objects can be recycled after the callback. A plain mutable object hides both constraints. Reproduce the observed failure with a faithful contract fixture, verify the correction there, and then run the complete native control. Treat mock coverage, native lifecycle coverage and success in the target as separate evidence.

Test ownership and cleanup against that same contract. A setter may wrap a supplied callback and return the wrapper through its getter; comparing the getter with the supplied function can falsely report another owner. Retain the actual installed identity when the API supports it, preserve a truly different owner's replacement, and verify restoration. Source-supported disposal behavior can explain a successful detach, but it does not retrospectively turn a failed explicit cleanup acknowledgment into a passing result.

Treat stopping as an asynchronous transition when callbacks can still be running. A stop request can disable new work before the last callback finishes; it is not yet proof that listeners and replacements were removed. Retain the terminal cleanup result, yield until in-flight work drains within a bounded deadline, and make the receiver wait for that result before declaring success. Exercise this transition in the complete owned control rather than interpreting a successful session detach as every hook's explicit acknowledgment.
...
Compare a quiet interval with the interaction that fails. Bounded cumulative counts taken before a press, while held, and after release can expose a dependency used only during input, even when rendering continues normally. Observe input delivery separately so a missing event cannot masquerade as a missing dependency. Such a correlation selects the next owned control; it does not prove the dependency's purpose or that its result is incorrect. Test the newly implicated operation with known inputs before widening a repair.
...
Record participation separately from installation: selection, eligible operations, substitutions actually returned, fallbacks and invalidations. If an installed adapter performs no substitutions, an unchanged screen does not show that its correction failed. Inspect why the expected operations were ineligible. A control may omit concurrent activity that changes the observer's bookkeeping without changing the target resource. Add that interleaving to the owned control and distinguish unrelated resources from changes to the selected resource's lifetime.

For an adapter, test both the corrected case and the untouched fallback with owned inputs before applying it to the target. Match only the state and operation sequence demonstrated by the control. Pass through unknown, stale or partially tracked state; invalidate inferred state after unsupported operations instead of fabricating an answer. Include these fallback cases in the owned control so a narrow correction cannot silently become a general replacement. Bind an intervention to its declared process, context and interval, stop substitutions at the chosen boundary, and separately acknowledge removal of hooks. A successful isolated correction is still not proof that it repairs the target's visible failure.

Budget observation around the interaction it must answer. Estimate startup and navigation time from previous runs, keep event-volume caps separate from lifetime, and record whether the decisive input occurred before cleanup. If the window ends before that input, preserve the successful startup evidence and label the interaction untested. A deliberately longer bounded next run tests the missing journey; repeating setup without covering that boundary adds little information.
...
Apply this reasoning to the observation tool's dependencies too. An unavailable container service need not invalidate a healthy target session. Preserve the required client/server version and API contract, inspect compatible architectures in existing local runtimes, and test the smallest supported alternative before installing another stack. Verify import, the operations the receiver actually uses, and then a bounded connection separately. A package filename alone is insufficient evidence of ABI compatibility; use its public packaging contract and an actual execution. Keep shared services and the target unchanged while testing an independent client route when possible.
