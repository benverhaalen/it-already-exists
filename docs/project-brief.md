...
Updated September 13, 2026. This document distinguishes the intended product from untested architecture proposals.
...
- Inputs: user-supplied references plus references the agent discovers. Websites, APKs, installed desktop applications, executables, recordings, repositories, APIs, and other observable surfaces are in scope.
...
2. They supply a reference URL or local recording and explain what appeals to them. The initial state is the project's normal entry screen, not an isolated demo component.
3. The integration reads the relevant project context, identifies quality gaps, and studies the supplied reference. It finds additional evidence for a specific missing behavior, such as recovery from a failed save.
...
This proposed demonstration tests reference discovery, transfer and application together. A subsequent native app or APK case tests whether the shared representation extends beyond browser references.
