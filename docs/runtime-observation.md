...
If a visible control does not respond, validate the input producer before inferring an application rule. Within the selected access profile, compare the event that reaches the intended view with the documented gesture contract: coordinates and viewport, press/release order, held duration, and the timestamp identifying the initial press. A tool named “tap” or “swipe” need not generate the same stream as a finger, especially on an old framework. A view returning “handled” establishes delivery at that boundary, not acceptance by the intended control. Compare a known working action and change one producer property at a time; record the resulting screen before claiming an input fix.
...
Advance the recorded phase before allocating capture directories or preparing a transfer. A storage error during capture setup must retain the completed input acknowledgment; reporting only that the command failed can cause a duplicate action. If the evidence journal also becomes unwritable, preserve the error and known action state through an already available output channel. Treat a partial journal as partial evidence and retain its original bytes.

Probe cleanup must remain possible when logging fails. Keep lifecycle acknowledgments in memory independently of file writes, attempt each owned cleanup step, and report missing durable evidence without turning it into success. A written stop request is not a cleanup acknowledgment. If the client infrastructure disappears, inspect the target and end the narrowly owned instrumented process when needed to remove uncertain hooks; preserve its state and do not restart shared infrastructure without understanding its ownership.
...
When moving a reference between operating systems, compare source names and file types before trusting the copy. A case-sensitive source can contain both `Config.json` and `config.json`; copying it onto a case-insensitive volume can merge them even when the transfer reports success. Preserve such state in an archive or a filesystem with matching name semantics, and inspect archive members without extracting them onto the incompatible volume. Check the source inventory against the saved members and read the saved payloads before depending on the backup. A readable archive header or an exit code alone does not establish a complete snapshot. Preserve symbolic-link metadata explicitly and verify its target without following it into unrelated files. Distinguish archive payload hashes from a separate comparison against source payload hashes; they provide different checks.
...
Qualify trace coverage for the property and phase being inferred. A recorder can remain connected while one channel has exhausted its event budget. An uncapped image-upload trace does not fill a missing draw-state trace, and an early state setter does not establish the state at a later screen. Record the first/last covered event, cap and dropped count separately for each channel. If repeated calls crowd out the decisive transition, retain repetition counts and bounded representative values, or arm a fresh observer at that transition. State inherited before attachment remains unknown unless the permitted evidence establishes it. Do not broaden a repair merely because its earlier success makes it familiar.

Classify visible defects before adopting them. Preserve the raw reference frame, identify the affected regions, and distinguish intended artwork, system overlays and unresolved observation defects. Use unaffected properties where supported; leave the damaged property unresolved until another observation or a controlled repair distinguishes the explanations. Reproducing an emulator's opaque rectangle would improve a naive screenshot score while making the recreated product less faithful.

Do not assign meaning to a data column solely because its values look plausible. Compare a proposed mapping against multiple visible cases, including one where competing mappings predict different results. If a displayed score matches several formulas, keep those hypotheses separate and select the next reference state to distinguish them. A complete catalog can be supported while its display transformation is still unknown.

Fit a candidate interpretation on one observation, then predict another without retuning it. Independently adjusting every example can conceal the wrong rule. When silhouettes are symmetric, inspect asymmetric shading, texture details or a nonsquare example that separates the alternatives. Record which measurements selected the model and which were held aside to check its prediction; a good fit supports only the tested transform, mapping or behavior.
...
Specify a visual transition as retained, removed and introduced content. Compare the complete before/after frames: a confirmation message can replace one panel while the heading, instructions and footer remain visible. Carry those retained elements into the implementation and its visual acceptance criteria. Assertions that only find the new message or reach the next screen can pass despite missing context. When the comparison reveals an omission, update the screen specification and the relevant checks together; a passing interaction test cannot overrule visible evidence.
